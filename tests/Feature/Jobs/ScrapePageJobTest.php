...
class ScrapePageJobTest extends TestCase
{
    ...
    public function test_creates_snapshot_with_failed_status_on_http_4xx(): void
    {
        ...
        $this->assertSame($entity->id, $snapshot->page_id);
    ...
    public function test_stops_retrying_after_max_attempts_and_creates_snapshot(): void
    {
        ...
        $this->assertSame(1, $entity->snapshots_count);
        $this->assertDatabaseCount('snapshots', 1);
        $latestSnapshot = Snapshot::where('page_id', $entity->id)->orderByDesc('version')->first();
    ...
    public function test_mark_entity_failed_clears_next_scrape_at_when_attempts_reaches_job_max(): void
    {
        ...
        $job = new class($entity) extends ScrapePageJob {
            public function exposeMarkPageFailed(): void
            {
                $this->markPageFailed();
            }
        };
    ...
    public function test_creates_linked_entities_with_correct_data_same_host_only(): void
    {
        ...
        $newEntity = Page::where('source_id', $source->id)->where('url', 'https://example.com/new-page')->first();
    ...
    public function test_does_not_duplicate_linked_entity_when_url_already_exists_for_source(): void
    {
        ...
        $existingCount = Page::where('source_id', $source->id)->where('url', 'https://example.com/existing')->count();
        $this->assertSame(1, $existingCount);
        $newOne = Page::where('source_id', $source->id)->where('url', 'https://example.com/new-one')->first();
    ...
    public function test_success_image_flow_skips_html_parsing_and_enriches_via_file_vision(): void
    {
        ...
        $job = new class($entity, $pngBody) extends ScrapePageJob {
            public function __construct(Page $page, private string $pngBytes)
            {
        ...
        $entity->refresh();
        $this->assertSame(ScrapingStatus::SUCCESS->value, $entity->scraping_status->value);
        $this->assertSame(ScrapableType::IMAGE, $entity->type);
