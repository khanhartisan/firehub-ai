...
class UpdateFileToolTest extends TestCase
{
    ...
    public function test_validation_fails_when_update_file_data_is_missing(): void
    {
        $user = User::factory()->create();
        $channel = $this->createFlyCmsChannel($user, 'Main Blog');
    ...
    public function test_returns_error_when_update_file_data_is_empty(): void
    {
        $user = User::factory()->create();
        $channel = $this->createFlyCmsChannel($user, 'Main Blog');
        ...
        $response->assertHasErrors(['Provide at least one field in update_file_data.']);
    }
    ...
    {
        $response = AppServer::tool(UpdateFileTool::class, [
