...
class ListMenusToolTest extends TestCase
{
    use RefreshDatabase;

    public function test_lists_menus_for_channel_website(): void
    {
        $user = User::factory()->create();
        $channel = $this->createFlyCmsChannel($user, 'Main Blog', '01J00000000000000000000001');

        $response = AppServer::actingAs($user)->tool(ListMenusTool::class, [
            'channel_id' => $channel->id,
        ]);

        $response
            ->assertOk()
            ->assertSee('Found 2 menus')
            ->assertName('platform-manager--flycms--list-menus-tool')
            ->assertDescription('List FlyCMS menus for the website linked to the given channel.')
    ...
    {
        $response = AppServer::tool(ListMenusTool::class, [
