...
class ShowMenuToolTest extends TestCase
{
    use RefreshDatabase;

    public function test_shows_menu_for_channel_website(): void
    {
        $user = User::factory()->create();
        $channel = $this->createFlyCmsChannel($user, 'Main Blog', '01J00000000000000000000001');

        $response = AppServer::actingAs($user)->tool(ShowMenuTool::class, [
        ...
        $response
            ->assertOk()
            ->assertSee('Menu details')
            ->assertName('platform-manager--flycms--show-menu-tool')
            ->assertDescription('Show a FlyCMS menu by ID for the website linked to the given channel.')
            ->assertStructuredContent(function ($json): void {
                $json->where('id', '01J00000000000000000000011')
                    ...
                    ->where('items', fn (mixed $items): bool => collect($items)
                        ->map(fn (mixed $item): array => (array) json_decode(json_encode($item), true))
                        ->pluck('text')
                        ->all() === ['Home', 'About'])
                    ->has('created_at')
                    ->has('updated_at')
    ...
    {
        $response = AppServer::tool(ShowMenuTool::class, [
