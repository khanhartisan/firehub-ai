...
class DeletePageToolTest extends TestCase
{
    use RefreshDatabase;

    public function test_deletes_page_for_channel_website(): void
    {
        $user = User::factory()->create();
        $channel = $this->createFlyCmsChannel($user, 'Main Blog', '01J00000000000000000000001');

        $response = AppServer::actingAs($user)->tool(DeletePageTool::class, [
        ...
        $response
            ->assertOk()
            ->assertSee('Successfully deleted page [01J00000000000000000000041]')
            ->assertName('platform-manager--flycms--delete-page-tool')
            ->assertDescription('Delete a FlyCMS page on the website linked to the given channel.')
            ->assertStructuredContent(function ($json): void {
                $json->where('page_id', '01J00000000000000000000041');
            });

        $this->assertNull(FlyCms::showPage('01J00000000000000000000041'));
        $this->assertCount(1, FlyCms::listPages('01J00000000000000000000001'));
    }
    ...
    {
        $response = AppServer::tool(DeletePageTool::class, [
