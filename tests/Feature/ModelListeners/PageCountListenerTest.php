...
class PageCountListenerTest extends TestCase
{
    use RefreshDatabase;

    private function pageCountFor(Source|Vertical|Tag $countable, ScrapableType $scrapableType, ScrapingStatus $scrapingStatus): int
    {
        $record = PageCount::query()
            ->where('countable_type', $countable->getMorphClass())
            ->where('countable_id', $countable->id)
            ->where('scrapable_type', $scrapableType)
    ...
    public function test_entity_tags_synced_then_synced_again_updates_counts(): void
    {
        ...
        $entity = $this->createPage($source, [
            'type' => ScrapableType::IMAGE,
            'scraping_status' => ScrapingStatus::SUCCESS,
        ]);
        $entity->tags()->sync([$tag1->id, $tag2->id]);

        $this->assertSame(1, $this->pageCountFor($tag1, ScrapableType::IMAGE, ScrapingStatus::SUCCESS));
        $this->assertSame(1, $this->pageCountFor($tag2, ScrapableType::IMAGE, ScrapingStatus::SUCCESS));
        $this->assertSame(0, $this->pageCountFor($tag3, ScrapableType::IMAGE, ScrapingStatus::SUCCESS));
        ...
        $tag2->refresh();
        $tag3->refresh();
        $this->assertSame(0, $this->pageCountFor($tag1, ScrapableType::IMAGE, ScrapingStatus::SUCCESS));
        $this->assertSame(1, $this->pageCountFor($tag2, ScrapableType::IMAGE, ScrapingStatus::SUCCESS));
        $this->assertSame(1, $this->pageCountFor($tag3, ScrapableType::IMAGE, ScrapingStatus::SUCCESS));
    }
    ...
    public function test_entity_verticals_synced_increments_vertical_counts(): void
    {
        ...
        $this->assertSame(1, $this->pageCountFor($source, ScrapableType::DOCUMENT, ScrapingStatus::PENDING));
        $this->assertSame(1, $this->pageCountFor($vertical1, ScrapableType::DOCUMENT, ScrapingStatus::PENDING));
        $this->assertSame(1, $this->pageCountFor($vertical2, ScrapableType::DOCUMENT, ScrapingStatus::PENDING));
    }
    ...
    public function test_entity_created_without_verticals_only_updates_source_count(): void
    {
        ...
        $this->assertSame(1, $this->pageCountFor($source, ScrapableType::IMAGE, ScrapingStatus::SUCCESS));
        $this->assertSame(0, $this->pageCountFor($vertical, ScrapableType::IMAGE, ScrapingStatus::SUCCESS));
    }
    ...
    public function test_scrapable_type_updated_decrements_old_and_increments_new_on_source_and_verticals(): void
    {
        ...
        $vertical = $this->createVertical();
        $entity = $this->createPage($source, [
            'type' => ScrapableType::UNCLASSIFIED,
            'scraping_status' => ScrapingStatus::PENDING,
        ]);
        $entity->verticals()->attach($vertical->id);

        $this->assertSame(1, $this->pageCountFor($source, ScrapableType::UNCLASSIFIED, ScrapingStatus::PENDING));
    ...
    public function test_scrapable_type_and_scraping_status_both_updated_decrements_old_and_increments_new(): void
    {
        ...
        $source->refresh();
        $this->assertSame(0, $this->pageCountFor($source, ScrapableType::UNCLASSIFIED, ScrapingStatus::PENDING));
        $this->assertSame(1, $this->pageCountFor($source, ScrapableType::DOCUMENT, ScrapingStatus::FAILED));
    }
